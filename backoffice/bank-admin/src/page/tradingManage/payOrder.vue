    ...
    <main class="fillcontain clear" v-if="this.$route.name == 'payOrder'">
      <section class="main">
        ...
        <el-form-item label="来源">
          ...
          <el-form-item label="通道">
            <el-select
              v-model="opyOrderForm.channel"
              placeholder="请选择"
              clearable
            >
              <el-option
                v-for="item in config.channels_deposit"
                :key="item.name"
                :label="item.desc"
                :value="item.name"
              >
        ...
        <el-button size="small" type="primary" @click="fillOrder">
          人工补单
        </el-button>
      </section>
      ...
      <section class="main">
        <el-radio-group v-model="activeName" size="medium">
          ...
          <el-radio-button label="30">支付成功</el-radio-button>
          <el-radio-button label="40">支付失败</el-radio-button>
        </el-radio-group>
        ...
        >
          ...
          <el-table-column align="center" prop="mch_id" label="通道商户号">
          </el-table-column>
          <el-table-column align="center" prop="channel" label="通道">
          </el-table-column>
              ...
              <el-dropdown style="margin-left:10px;" @command="handleCommand">
                ...
                <el-dropdown-menu slot="dropdown">
                  <el-dropdown-item
                    ...
                    :command="{ id: scope.row, type: 'orderNotify' }"
                    >补发通知</el-dropdown-item
                  >
                  <el-dropdown-item
                    :command="{ id: scope.row, type: 'complete' }"
                    >设为完成</el-dropdown-item
                  >
                </el-dropdown-menu>
              </el-dropdown>
            </div>
...
<script>
import { configGetunt, depositOrderList, tradeRefuseOrderNotify, depositOrderListExport, tradeRefuseManuallyDone } from '@/api/getData'
import { getTimeForm } from '@/config/mUtils'
  ...
  methods: {
    ...
    async init () {
      ...
      this.withdrawList = {
        ...
        merchant_name: this.opyOrderForm.name,
        channel: this.opyOrderForm.channel,
        page_size: 10,
        page_index: this.currentPage,
        begin_time: getTimeForm(this.opyOrderForm.optionTime[0]),
        end_time: getTimeForm(this.opyOrderForm.optionTime[1]),
        state: this.activeName
      }
      const data = await depositOrderList(this.withdrawList)
      if (data) {
    ...
    // 下载订单
    async load () {
      const data = await depositOrderListExport(this.withdrawList)
      if (data) {
        console.warn(data)
      }
    },
    ...
    async handleCommand (command) {
      ...
      // 补发通知
      if (command.type == 'orderNotify') {
        const data = await tradeRefuseOrderNotify({ order_id: command.id.order_id, order_type: 'DEPOSIT' })
        ...
        // 设为完成
        this.$prompt('请输入备注', '是否将此订单的状态设置为支付成功！', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          inputPattern: /^.{5,100}$/,
          inputErrorMessage: '备注信息最小5位，最大100位'
        }).then(({ value }) => {
          ManuallyDone(value)
        }).catch(() => { });
        let ManuallyDone = async (value) => {
          const data = await tradeRefuseManuallyDone({ order_id: command.id.order_id, comment: value })
          if (data) {
            command.id.state = '充值成功'
            command.id.done_time = getTimeForm(new Date())
            this.$message({
              type: 'success',
              message: '设置成功'
            });
